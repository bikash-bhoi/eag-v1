...
def calculate(expression: str) -> TextContent:
    ...
    try:
        ...
        return TextContent(
            type="text",
            text=f"Error: {str(e)} for Expression: {expression}"
        )
...
@mcp.tool()
async def draw_rectangle(x1: int=300, y1: int=300, x2: int=700, y2: int=700) -> TextContent:
    ...
    try:
        ...
        canvas.press_mouse_input(coords=(x1+1280, y1))
        canvas.move_mouse_input(coords=(x2+1280, y2))
        canvas.release_mouse_input(coords=(x2+1280, y2))
...
@mcp.tool()
async def add_text_to_paint(text: str) -> TextContent:
    """Add text in Paint"""
    global paint_app
    try:
        if not paint_app:
...
@mcp.tool()
async def open_paint() -> TextContent:
